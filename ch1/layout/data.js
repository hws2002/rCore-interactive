...
const STEPS = [
  ...
  {
    title: 'entry.asm: _start → la sp, boot_stack_top',
    file: 'os/src/entry.asm',
    code: CODE_ENTRY, line: 4,
    sections: ['text', 'rodata', 'data', 'stack'],
    highlight: ['text', 'stack'],
    symbols: { '_start': '0x80200000', sp: '0x80214000 (boot_stack_top)' },
    desc: 'entry.asm의 _start가 0x80200000에서 실행됩니다.\n\n`la sp, boot_stack_top`\n• la = Load Address (PC 상대 주소 계산)\n• boot_stack_top ≈ 0x80214000 (linker.ld에서 결정)\n• sp(스택 포인터)를 스택 최상단으로 초기화\n\nsp 초기화 없이 call 명령을 실행하면 sp가 0이므로\n첫 번째 함수 호출에서 즉시 크래시!\n\n`.section .text.entry`로 .text.entry 섹션에 배치 → 0x80200000 보장.',
    detail: 'RISC-V에서 함수 호출(call/jal)은 반환 주소를 ra(x1)에 저장하고 스택에 저장합니다. sp가 유효하지 않으면 저장 즉시 fault.',
  },
  {
    title: 'entry.asm → .bss 섹션 완성',
    file: 'os/src/entry.asm + linker.ld',
    code: CODE_ENTRY, line: 4,
    sections: ['text', 'rodata', 'data', 'stack', 'bss'],
    highlight: ['text', 'stack'],
    symbols: {
      '_start':   '0x80200000',
      sp:         '0x80214000 (boot_stack_top)',
      sbss:       '~0x80214000',
      ebss:       '~0x80215000',
    },
    desc: 'entry.asm의 _start가 .text.entry에 배치됩니다.\n\n빌드 타임에 결정되는 최종 레이아웃:\n• _start @ 0x80200000 (링커 보장)\n• sp 초기값 = boot_stack_top = 0x80214000\n• sbss ~ ebss: 런타임에 clear_bss()가 0으로 초기화\n\n.bss는 ELF 파일에 크기만 기록되고\n실제 초기화는 런타임에 일어납니다.',
    detail: 'ELF .bss 섹션: 크기(size)만 저장, 초기값 없음. 로더 또는 커널 자신이 0으로 채워야 합니다.',
  },
  {
